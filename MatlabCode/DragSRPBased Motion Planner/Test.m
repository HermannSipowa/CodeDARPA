...
X0_Chief2  = Xnom(end,:)';
[~, Xnom0] = ode113(@(t,X)DragSRPChiefMotionODE(t,X,Target),Ttraj1,X0_Chief1,options);
[~, Xnomf] = ode113(@(t,X)DragSRPChiefMotionODE(t,X,Target),Ttraj2,X0_Chief2,options);
...
for ll = 1
    ...
    tic
    [~, Xrel]  = ode113(@(t,x)DragSRPNonDimentionalized_FlatPlateOde(t,x,Target,Chasser),Tnorm,X0,options);
    toc
    for l = 1
        % fh2 = figure;
        % view(-73,7)
...
for ll = 1
    ...
    % opts = odeset('RelTol',2.22045e-14,'AbsTol',2.22045e-20,'NormControl','on');
    ...
    x = [0 logspace(-4,log10(T),xpoints-1)]; % linspace(0,T,xpoints);
