function varargout=arrow3d(start,stop,Color,p,n,ang,ltype)
%   Plot 3d Arrows
%
...
%   Color:      Matrix containing RGB color. Color(1,:) is color for the body and Color(1,:)
%               is the color for the head
%
%   Example:
...
if nargin<3 || isempty(Color)
    Color(1,:) = [145,90,7]/255;
    Color(2,:) = [11 131 222]/255;
end
...
if nargin<6 || isempty(ang)
    ang=15;
end
if nargin<7 || isempty(ltype)
    ltype='cylinder';
end
...
if strcmp(ltype,'line')
    ...
    for i=1:length(dis)
        ...
        hlines(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',Color(1,:));
...
for i=1:length(dis)
    ...
    [rx,ry,rz] = rotatedata(xi,yi,zi,nvec(i,:),cosrang(i),[0,0,0]);
    cx=start(i,1)+rx;cy=start(i,2)+ry;cz=start(i,3)+rz;
    hheads(i)=surf(cx,cy,cz,'edgecolor','none','facecolor',Color(2,:));
...
if nargout>1 varargout{2}=[hheads;hhgrd];
end
if nargout>0 varargout{1}=[hlines;hlgrd];
end